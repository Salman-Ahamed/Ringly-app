...
class CallerIdOverlayView(private val context: Context) : OverlayRenderer {
    ...
    private fun buildView(card: CallerIdCard): View {
        ...
        val numberText = view.findViewById<TextView>(R.id.overlay_caller_number)
        if (name == card.number) {
            numberText.visibility = View.GONE
        } else {
            numberText.text = card.number
        }
        view.findViewById<TextView>(R.id.overlay_owner_text).text = card.sourceLabel
